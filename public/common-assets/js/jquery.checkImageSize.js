		        ...
		        if ((file = this.files[0])) {
		            ...
		            img.onload = function () {
		                ...
		                if (imgWidth < minWidth || imgHeight < minHeight) {
		                    validImage = false;
		                    if (opts.showError) {
                                        toastr["error"]('Please select an image with at-least ' + minWidth + 'px width & ' + minHeight + 'px height!');
		                    }
		                }
		                ...
		                if (imgWidth > maxWidth || imgHeight > maxHeight) {
		                    validImage = false;
		                    if (opts.showError) {
                                        toastr["error"]('Please select an image of maximum ' + maxWidth + 'px width & ' + maxHeight + 'px height!');
		                    }
		                }
